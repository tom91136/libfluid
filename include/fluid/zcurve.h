...
static inline size_t coordAtZCurveGridIndex0(size_t index) {
	return uninterleave((index) & 0x9249249);
}

static inline size_t coordAtZCurveGridIndex1(size_t index) {
	return uninterleave((index >> 1) & 0x9249249);
}

static inline size_t coordAtZCurveGridIndex2(size_t index) {
	return uninterleave((index >> 2) & 0x9249249);
}

static inline size_t zCurveGridIndexAtCoord(size_t x, size_t y, size_t z) {
	x = (x | (x << 16)) & 0x030000FF;
	x = (x | (x << 8)) & 0x0300F00F;
	x = (x | (x << 4)) & 0x030C30C3;
	x = (x | (x << 2)) & 0x09249249;
